...
import { ChevronRight, HeartPulse, Loader2, Search, ChevronLeft, AlertCircle, Stethoscope, Check, X, Save, RotateCcw } from "lucide-react";
...
export default function DiseasePrediction() {
  ...
  const { data: symptomsList } = useQuery<string[]>({
  ...
  const { data: historyRaw, isLoading: histLoading } = useQuery<BackendPrediction[]>({
      ...
      <div className="mt-10 grid gap-8 lg:grid-cols-[minmax(0,1fr)_280px]">
        <main>
          <div className="mb-6 flex flex-wrap items-center gap-2 text-sm text-textSecondary">
            {[1, 2, 3].map(i => (
              <span key={i} className="flex items-center gap-2">
                <span className={`grid h-7 w-7 place-items-center rounded-full ${step >= i ? "bg-textPrimary text-white" : "border border-borderLight"}`}>{i}</span>
          ...
          <form onSubmit={runPrediction} className="space-y-6">
            <section className="border border-borderLight bg-white p-5 md:p-8">
                ...
                <>
                  {selected.length > 0 && (
                    <div className="mb-4 flex flex-wrap gap-2">
                      {selected.map(s => (
                        <span key={s} className="inline-flex items-center gap-1.5 rounded-full bg-textPrimary px-3 py-1.5 text-xs text-white">
                          {s}
                          <button type="button" onClick={() => toggleSymptom(s)} className="hover"><X className="h-3 w-3" /></button>
                        </span>
                      ))}
                    </div>
                  )}
                  <div className="relative mb-4">
                    ...
                    <input value={symptomSearch} onChange={e => setSymptomSearch(e.target.value)} className="h-11 w-full rounded-md border border-borderLight pl-9 pr-3 text-sm outline-none focus:ring-1 focus:ring-textPrimary" placeholder="Search symptoms..." />
                  </div>
                  <div className="flex max-h-60 flex-wrap gap-2 overflow-y-auto">
                    {filteredSymptoms.map(s => (
                      <button key={s} type="button" onClick={() => toggleSymptom(s)} disabled={!selected.includes(s) && selected.length >= 10} className={`inline-flex items-center gap-1.5 rounded-full px-3.5 py-1.5 text-sm transition ${selected.includes(s) ? "bg-textPrimary text-white" : "border border-borderLight hover:bg-gray-50 disabled:opacity-30"}`}>
                  ...
                  {!symptomsList && <div className="flex items-center justify-center py-8"><Loader2 className="h-6 w-6 animate-spin text-textSecondary" /></div>}
                  {symptomsList && filteredSymptoms.length === 0 && !isPredicting && <p className="py-4 text-center text-sm text-textSecondary">No symptoms match your search.</p>}
                </>
              )}
            </section>
            ...
            <section className="border border-borderLight bg-white p-5 md:p-8">
                    ...
                    {saveSuccess ? (
                      ...
                      <div className="space-y-3">
                        <label className="flex items-center gap-2 text-sm">
                          ...
                          <select value={linkPetId} onChange={e => setLinkPetId(e.target.value)} className="h-9 rounded border border-borderLight px-2 text-xs">
                            <option value="">Don't link</option>
                            {pets?.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                          </select>
                        </label>
                        <Button type="button" onClick={saveResult} disabled={isSaving} size="sm">
              ...
              <div className="mt-5 flex justify-end gap-3">
                ...
                <Button type="submit" disabled={isPredicting || !species || selected.length === 0}>
        ...
        <aside>
          <div className="sticky top-24 rounded-xl border border-borderLight bg-white p-5">
            <div className="flex items-center justify-between">
              <h3 className="font-medium">History</h3>
              <button onClick={() => queryClient.invalidateQueries({ queryKey: ["predictions", "all"] })} className="text-textSecondary hover:text-textPrimary"><RotateCcw className="h-4 w-4" /></button>
            </div>

            <div className="mt-4 space-y-2">
              <label className="relative block">
                <Search className="absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-textSecondary" />
                <input value={historySearch} onChange={e => setHistorySearch(e.target.value)} className="h-9 w-full rounded-md border border-borderLight pl-8 pr-2 text-xs outline-none" placeholder="Search diseases or species" />
              </label>
              <select value={filterSpecies} onChange={e => setFilterSpecies(e.target.value)} className="h-9 w-full rounded-md border border-borderLight px-2 text-xs">
            ...
            <div className="mt-4 space-y-2">
              {histLoading && <div className="space-y-2">{[1, 2, 3].map(i => <div key={i} className="h-14 animate-pulse rounded-md bg-gray-100" />)}</div>}
              {!histLoading && filteredHistory.length === 0 && <p className="py-4 text-center text-xs text-textSecondary">No predictions yet</p>}
              {filteredHistory.map(h => (
                <div key={h.id} className="rounded-md border border-borderLight p-3">
