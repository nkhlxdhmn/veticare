...
def list_pets(
    ...
    return get_pets_by_owner(supabase, current_user["id"], offset=offset, limit=limit)


@router.post("", response_model=PetResponse, status_code=status.HTTP_201_CREATED)
def add_pet(body: PetCreate, current_user: CurrentUser, supabase: SupabaseClient) -> dict:
    """Create a new pet for the authenticated user."""
    return create_pet(supabase, current_user["id"], body)


@router.get("/{pet_id}", response_model=PetResponse)
def read_pet(pet_id: uuid.UUID, current_user: CurrentUser, supabase: SupabaseClient) -> dict:
    """Return a single pet if it belongs to the authenticated user."""
    return _get_pet_or_404(supabase, pet_id, current_user["id"])


@router.patch("/{pet_id}", response_model=PetResponse)
def patch_pet(pet_id: uuid.UUID, body: PetUpdate, current_user: CurrentUser, supabase: SupabaseClient) -> dict:
    """Update a pet's fields."""
    _get_pet_or_404(supabase, pet_id, current_user["id"])
    result = update_pet(supabase, pet_id, body)
    if not result:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Pet not found")
    return result
...
def remove_pet(pet_id: uuid.UUID, current_user: CurrentUser, supabase: SupabaseClient) -> None:
    """Delete a pet and its cascaded records."""
    _get_pet_or_404(supabase, pet_id, current_user["id"])
    delete_pet(supabase, pet_id)
